...
function buildHorizonMountains(waveSeed = 1) {
  ...
  for (let i = 0; i < count; i++) {
    ...
    const r = baseR + 100 + (i % 6) * 38;
    const bx = cx + Math.cos(a) * r;
    const bz = cz + Math.sin(a) * r;
    const h = 48 + (i % 5) * 18 + (i % 3) * 12;
    const w = 110 + (i % 4) * 28;
...
export function createFlightGround() {
  ...
  const ring = new THREE.Mesh(
    new THREE.RingGeometry(ARENA.radius * 0.95, ARENA.radius * 1.01, 48),
